-- Update approve_vendor_request function to create referral records
-- When a vendor is approved with a referral code, create a referral entry
...
BEGIN
  ...
  -- Create referral record if referred_by_code was provided
  IF v_request.referred_by_code IS NOT NULL AND v_request.referred_by_code != '' THEN
    -- Find the referrer vendor by promo_code
    SELECT id INTO v_referrer_id
    FROM vendors
    WHERE promo_code = v_request.referred_by_code;

    -- Only create referral if we found a valid referrer
    IF v_referrer_id IS NOT NULL THEN
      INSERT INTO referrals (
        ...
        v_referrer_id,
        v_vendor_id,
        v_request.referred_by_code,
        0, -- Commission calculation can be added later
        'completed',
        NOW()
      );
    END IF;
...
COMMENT ON FUNCTION approve_vendor_request IS 'Approves a vendor request, creates Supabase Auth user, vendor record with unique promo code, initial subscription, and referral record if applicable';
